...
// ══════════════════════════════════════════════════════════
// PREPARAR Y ABRIR TEST DEL EVALUADO DESDE ADMIN
// ══════════════════════════════════════════════════════════

document.addEventListener("DOMContentLoaded", function () {
    const btnPrepararTest = document.getElementById("btn-preparar-test-evaluado");
    const btnAbrirTestPreparado = document.getElementById("btn-abrir-test-preparado");

    if (btnPrepararTest) {
        btnPrepararTest.addEventListener("click", function () {
            const modalElement = document.getElementById("modalPrepararTest");

            if (!modalElement) {
                Swal.fire({
                    icon: "error",
                    title: "Modal no encontrado",
                    text: "Falta agregar el modal modalPrepararTest en index.html.",
    ...
    if (btnAbrirTestPreparado) {
        btnAbrirTestPreparado.addEventListener("click", function () {
            const estudiante = document.getElementById("prep-estudiante").value;
            const subtest = document.getElementById("prep-subtest").value;

            if (!estudiante || !subtest) {
                Swal.fire({
                    icon: "warning",
                    title: "Faltan datos",
                    text: "Selecciona el estudiante y el subtest antes de abrir la evaluación.",
                    confirmButtonText: "Entendido"
                });
                return;
            }

            const url = `./test.html?estudiante=${encodeURIComponent(estudiante)}&subtest=${encodeURIComponent(subtest)}`;
